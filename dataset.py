import torch
from transformers import AutoProcessor, AutoFeatureExtractor, WhisperFeatureExtractor
import os
import torch
...
class MyCollator:
    def __init__(self, audio_encoder_name, tokenizer):
        ...
        if "openai/whisper" in audio_encoder_name:
            # Use WhisperFeatureExtractor for Whisper models
            self.audio_processor = WhisperFeatureExtractor.from_pretrained(audio_encoder_name)
        else:
            # Use AutoFeatureExtractor for HuBERT/WavLM
    ...
    def __call__(self, batch):
        ...
        if waveform is not None:
            if "openai/whisper" in self.audio_encoder_name:
                # Process with WhisperFeatureExtractor
                ...
                # Process with HuBERT/WavLM processor
                mel = self.audio_processor(
                    waveform.squeeze(), 
                    return_tensors="pt", 
                    sampling_rate=16000
